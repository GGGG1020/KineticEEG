...
sens=['AF3','F7','F3', 'FC5', 'T7', 'P7', 'O1', 'O2','P8', 'T8', 'FC6', 'F4','F8', 'AF4']
mylist={'AF3':[],'F7':[],'F3':[], 'FC5':[], 'T7':[], 'P7':[], 'O1':[], 'O2':[],'P8':[], 'T8':[], 'FC6':[], 'F4':[],'F8':[], 'AF4':[]}
userID            = c_uint(0)
nSamples   = c_uint(0)
nSam       = c_uint(0)
    ...
    def mainloop(self):
        ...
        while not initial:
            ...
            if state == 0:
                ...
                libEDK.EE_EmoEngineEventGetUserId(self.eEvent, user)
            ...
            if readytocollect==True:    
                ...
                libEDK.EE_DataGetNumberOfSample(self.hData,nSamplesTaken)
             #   print("Updated :",nSamplesTaken[0])
                if nSamplesTaken[0] >= 512:
                    ...
                    for sampleIdx in range(nSamplesTaken[0]): 
                        for i in range(14): 
                            ...
                            mylist[sens[i]].append(arr[sampleIdx])
                        if len(mylist["AF3"])==512:
                            a.send(pickle.dumps(mylist))
                            mylist=mylist.fromkeys(mylist, 0)
                            initial=True
                            break
                if a.recv(1024)==b'terminate':
                    self.free()
            #time.sleep()
        while (1):
            state = libEDK.EE_EngineGetNextEvent(self.eEvent)
